...
wsm_model_zscore <- function(dataset, criteria, criteria_weights, criteria_type) {
  ...
  zscore_ben <- function(x) {
    norm = (x - mean(x, na.rm = TRUE)) / (sd(x, na.rm = TRUE))
    return(norm)
  }
  
  zscore_NONben <- function(x) {
    norm = 1 - ((x - mean(x, na.rm = TRUE)) / (sd(x, na.rm = TRUE)))
    return(norm)
  }
